...
class LitAuto(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if class_weights is not None:
            class_weights = class_weights.to(y_hat.device)
            sample_weights = class_weights[y.long()]
        else:
            sample_weights = None
        loss = nn.functional.binary_cross_entropy_with_logits(y_hat, y.float(), weight=sample_weights)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.log("valid_S", S, prog_bar=True, sync_dist=True)
