...
class LitModel(object):
    ...
    def fit(self, train_data: Dataset, valid_data: Optional[Dataset] = None, verbose=True):
        ...
        train_iter = DataLoader(
            self.train_set, self.__batchsize if self.__batchsize > 0 else len(self.train_set), shuffle=True,pin_memory=True  # type: ignore
        )
        if self.valid_set:
            valid_iter = DataLoader(
                self.valid_set, self.__batchsize if self.__batchsize > 0 else len(self.valid_set), shuffle=False,pin_memory=True  # type: ignore
            )
        else:
            valid_iter = None
    ...
    def test(self, test_data: Dataset, is_loader=False, verbose=True, ckpt_path="best"):
        if is_loader:
